...
fix_mkpkg_format_invalid() {
    if [[ $BUILD_DIR =~ "imm-nss" ]]; then
        if [ -f $BUILD_DIR/feeds/smpackage/v2ray-geodata/Makefile ]; then
            sed -i 's/VER)-\$(PKG_RELEASE)/VER)-r\$(PKG_RELEASE)/g' $BUILD_DIR/feeds/smpackage/v2ray-geodata/Makefile
        fi
        if [ -f $BUILD_DIR/feeds/smpackage/luci-lib-taskd/Makefile ]; then
            sed -i 's/>=1\.0\.3-1/>=1\.0\.3-r1/g' $BUILD_DIR/feeds/smpackage/luci-lib-taskd/Makefile
        fi
        if [ -f $BUILD_DIR/feeds/smpackage/luci-app-openclash/Makefile ]; then
            sed -i 's/PKG_RELEASE:=beta/PKG_RELEASE:=1/g' $BUILD_DIR/feeds/smpackage/luci-app-openclash/Makefile
        fi
        if [ -f $BUILD_DIR/feeds/smpackage/luci-app-quickstart/Makefile ]; then
            sed -i 's/PKG_VERSION:=0\.8\.16-1/PKG_VERSION:=0\.8\.16/g' $BUILD_DIR/feeds/smpackage/luci-app-quickstart/Makefile
            sed -i 's/PKG_RELEASE:=$/PKG_RELEASE:=1/g' $BUILD_DIR/feeds/smpackage/luci-app-quickstart/Makefile
        fi
        if [ -f $BUILD_DIR/feeds/smpackage/luci-app-store/Makefile ]; then
            sed -i 's/PKG_VERSION:=0\.1\.27-1/PKG_VERSION:=0\.1\.27/g' $BUILD_DIR/feeds/smpackage/luci-app-store/Makefile
            sed -i 's/PKG_RELEASE:=$/PKG_RELEASE:=1/g' $BUILD_DIR/feeds/smpackage/luci-app-store/Makefile
        fi
    fi
}
...
update_tcping() {
    local tcping_path="$BUILD_DIR/feeds/smpackage/tcping/Makefile"
...
update_menu_location() {
    ...
    local tailscale_path="$BUILD_DIR/feeds/smpackage/luci-app-tailscale/root/usr/share/luci/menu.d/luci-app-tailscale.json"
...
update_script_priority() {
    ...
    local mosdns_path="$BUILD_DIR/package/feeds/smpackage/luci-app-mosdns/root/etc/init.d/mosdns"
...
update_mosdns_deconfig() {
    local mosdns_conf="$BUILD_DIR/feeds/smpackage/luci-app-mosdns/root/etc/config/mosdns"
...
fix_quickstart() {
    local file_path="$BUILD_DIR/feeds/smpackage/luci-app-quickstart/luasrc/controller/istore_backend.lua"
...
update_oaf_deconfig() {
    local conf_path="$BUILD_DIR/feeds/smpackage/open-app-filter/files/appfilter.config"
    local uci_def="$BUILD_DIR/feeds/smpackage/luci-app-oaf/root/etc/uci-defaults/94_feature_3.0"
    local disable_path="$BUILD_DIR/feeds/smpackage/luci-app-oaf/root/etc/uci-defaults/99_disable_oaf"
...
update_geoip() {
    local geodata_path="$BUILD_DIR/package/feeds/smpackage/v2ray-geodata/Makefile"
...
fix_easytier_lua() {
    local file_path="$BUILD_DIR/package/feeds/smpackage/luci-app-easytier/luasrc/model/cbi/easytier.lua"
    if [ -f "$file_path" ]; then
...
fix_easytier_mk() {
    local mk_path="$BUILD_DIR/feeds/smpackage/luci-app-easytier/easytier/Makefile"
    if [ -f "$mk_path" ]; then
